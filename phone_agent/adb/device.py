...
def get_current_app(device_id: str | None = None) -> str:
    ...
    # Use bytes output to avoid locale decode errors on Windows/GBK consoles.
    result = subprocess.run(
        adb_prefix + ["shell", "dumpsys", "window"], capture_output=True, text=False
    )
...
def post_notification(
    title: str, text: str, device_id: str | None = None, tag: str = "autoglm", nid: str = "1"
) -> None:
    """
    Post a status notification on the device (Android 8+).

    Uses `cmd notification post -S bigtext` to show a small banner in shade,
    avoiding custom APK dependencies. Works best on API 26+.
    """
    adb_prefix = _get_adb_prefix(device_id)
    subprocess.run(
        adb_prefix
